<template>
  <div class="w3-modal w3-animate-opacity" style="display:block; padding-top:50px;" v-show="showModal === true">
    ...
    <div class="w3-container w3-col w3-light-grey w3-round-xxlarge w3-padding-16 w3-margin-bottom">
      ...
      <input :value="state.product.name" class="w3-input w3-round-xxlarge w3-border-0 w3-margin-bottom" type="text">
      <label  class="w3-left w3-margin-left">Type</label>
      <select class="w3-input w3-round-xxlarge w3-border-0 w3-margin-bottom">
        <option :value="state.product.type" disabled selected>{{ product.type}}</option>
        <option value="1">Option 1</option>
        <option value="2">Option 2</option>
        <option value="3">Option 3</option>
      </select>
      <label class="w3-left w3-margin-left">Cost</label>
      <input  :value="state.product.cost" class="w3-input w3-round-xxlarge w3-border-0 w3-margin-bottom" type="text">
      <label class="w3-left w3-margin-left">Location</label>
      <select class="w3-input w3-round-xxlarge w3-border-0 w3-margin-bottom">
        <option value="" disabled selected>{{state.product.location}}</option>
        <option value="1">Option 1</option>
        <option value="2">Option 2</option>
        <option value="3">Option 3</option>
      </select>
      <label class="w3-left w3-margin-left">Condition</label>
      <input  :value="state.product.condition" class="w3-input w3-round-xxlarge w3-border-0 w3-margin-bottom" type="text">
      <label class="w3-left w3-margin-left">Serial Number</label>
      <input  :value="product.serialNo" class="w3-input w3-round-xxlarge w3-border-0 w3-margin-bottom" type="text">
      <p><input  :value="product.damaged" class="w3-check " type="checkbox"> <label>Damaged</label></p>
...
export default {
  ...
  setup(props, context) {
    ...
    async function updateProduct() {
      state.product = await putProductDetail(state.product);
